...
export default function ParentCreate({
  ...
  async function create() {
    ...
    if (e || !data?.success) {
      setError(data?.error ?? e?.message ?? 'Unable to create Parent Login.')
      setBusy(false)
      return
    }
    setDone(true)
  ...
  return (
    <div className="admin-import-page">
      ...
      <div className="import-header">
        <div>
          <p className="eyebrow">ADMIN • CREATE PARENT</p>
          <h1>Create Parent Login</h1>
          <p>Create one parent account and link it to a student.</p>
        </div>
      </div>
      <div className="import-card">
        ...
        {done && (
          <div className="result-card">
            ...
            <strong>Parent Login created successfully.</strong>
            <span>Login ID: {loginId}</span>
            <span>The parent is linked to student {studentCode}.</span>
          </div>
        )}
        <button
