...
function AdminModuleActive({
  ...
  return (
    <button className="admin-module admin-module-active" onClick={onClick}>
...
function AdminPortal({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    const onPopState = (event: PopStateEvent) => {
      ...
      if (state?.schoolUniformApp === 'admin') {
        setTool(state.tool || 'menu')
        return
      }
      setTool('menu')
  ...
  useEffect(() => {
    ...
    if (window.history.state?.schoolUniformApp === 'admin' && window.history.state?.tool === tool)
      return
  ...
  async function chooseSchool(v: string) {
    setSchool(v)
    setBranch('')
    setBranches([])
    const client = supabase
        ...
        <div className="admin-content">
          <div className="admin-title">
            <div>
              ...
              <h1>Student & Parent Management</h1>
              <p>Create test accounts individually or import your student register in bulk.</p>
            </div>
          </div>
          <div className="admin-scope-card">
            <div>
              <span>School</span>
              <strong>{schools.find((x) => x.id === school)?.name ?? 'Select school'}</strong>
            </div>
            <div>
              <span>Branch</span>
              <strong>{branches.find((x) => x.id === branch)?.name ?? 'Select branch'}</strong>
            </div>
          </div>
          <div className="admin-selector-grid">
            <label>
              School
              <select value={school} onChange={(e) => void chooseSchool(e.target.value)}>
          ...
          <div className="admin-action-grid">
            <button onClick={() => setTool('home')}>
              <Home />
              <div>
                <strong>Homepage Editor</strong>
                <span>Control hero, sections, images, text and footer.</span>
              </div>
              <ArrowRight />
            </button>
            <button disabled={!school || !branch} onClick={() => setTool('parent')}>
              <UserRound />
              <div>
                <strong>Create Parent Login</strong>
                <span>Create one parent + student account for testing.</span>
              </div>
              <ArrowRight />
            </button>
            <button disabled={!school || !branch} onClick={() => setTool('import')}>
              <FileSpreadsheet />
              <div>
                <strong>Bulk Student Import</strong>
                <span>Upload Excel or CSV student records.</span>
              </div>
              <ArrowRight />
            </button>
          </div>
          <div className="admin-module-grid">
            <AdminModuleActive
              ...
              text="Manage school campuses and branch details."
              onClick={() => setTool('schools')}
            />
            <AdminModuleActive
              icon={<ShoppingBag />}
              title="Products & Variants"
              text="Manage shirts, pants, shoes, sizes and SKUs."
              onClick={() => setTool('products')}
            />
            <AdminModuleActive
              icon={<Package />}
              title="Uniform Packages"
              text="Build boys and girls packages from individual products."
              onClick={() => setTool('packages')}
            />
            <AdminModuleActive
              icon={<ClipboardList />}
              title="Orders"
              text="Review orders, statuses and customer details."
              onClick={() => setTool('orders')}
            />
            <AdminModuleActive
              icon={<CreditCard />}
              title="Payments"
              text="Track payment status and transaction references."
              onClick={() => setTool('payments')}
            />
            <AdminModuleActive
              icon={<ClipboardList />}
              title="Inventory"
              text="Monitor stock and inventory movements."
              onClick={() => setTool('inventory')}
            />
            <AdminModuleActive
              icon={<UserRound />}
              title="Parents & Students"
              text="Manage student profiles and parent relationships."
              onClick={() => setTool('students')}
            />
            <AdminModuleActive
              icon={<FileSpreadsheet />}
              title="Reports"
              text="Sales, inventory and school-level reporting."
              onClick={() => setTool('reports')}
            />
            <AdminModuleActive
              icon={<BookOpen />}
              title="Catalog Rules"
              text="Configure school-specific products and pricing."
              onClick={() => setTool('catalog')}
            />
            <AdminModuleActive
              icon={<Sparkles />}
              title="Coupons"
              text="Manage branch-specific promotions and discounts."
              onClick={() => setTool('coupons')}
            />
          </div>
        </div>
      </div>
