...
function CustomerPortal({schoolId,schoolName,branchName,branchId,studentId,page,setPage,onLogout}:{schoolId:string;schoolName:string;branchName:string;branchId:string;studentId:string;page:CustomerPage;setPage:(p:CustomerPage)=>void;onLogout:()=>void}){
  ...
  useEffect(()=>{
    const onPopState=(event:PopStateEvent)=>{
      ...
      window.history.replaceState({schoolUniformApp:'customer',screen:'page:dashboard',page:'dashboard'},'', '/app')
    }
  ...
  return <div className="portal">
    ...
    <div className="portal-layout">
      <aside className="sidebar"><div className="portal-brand"><div className="portal-brand-title">Parent Portal</div></div><div className="school-scope"><Building2 size={16}/><div><strong>{schoolName}</strong><span>{branchName}</span></div></div><nav>{nav.map(([key,label,Icon])=><button className={page===key?'active':''} onClick={()=>setPage(key)} key={key}><Icon size={18}/>{label}</button>)}</nav><button className="sidebar-logout" onClick={onLogout}><LogOut size={17}/> Logout</button></aside>
